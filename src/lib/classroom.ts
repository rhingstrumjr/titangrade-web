...
export async function attachFeedbackDocToSubmission(
  ...
  const separator = "\n" + "=".repeat(40) + "\n";
  const entryText = `Feedback for ${studentName} — ${timestamp}\n\n${feedbackText}\n${separator}\n`;

  if (!documentId) {
    // 1. Create the Google Doc if it doesn't exist
  ...
  // 2. Insert text into the Doc (always at the start to keep newest on top)
  ...
  // 3. Attach the Doc to the Google Classroom Submission ONLY if we just created it
  if (!existingDocId) {
